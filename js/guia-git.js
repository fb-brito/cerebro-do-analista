document.addEventListener('DOMContentLoaded', () => {
    ...
    const gitCommands = [
        ...
        { title: 'Cenário 1: Descartar uma branch com erro (antes do merge)', command: '# 1. Garante que você está na branch principal\ngit checkout main\n\n# 2. Força a exclusão da branch localmente (-D)\ngit branch -D nome-da-branch-com-erro\n\n# 3. Remove a branch também do repositório remoto\ngit push origin --delete nome-da-branch-com-erro', explanation: 'Use estes comandos quando você cometeu um erro em uma branch e quer simplesmente apagá-la, tanto localmente quanto no GitHub, para começar de novo. O -D maiúsculo força a exclusão local.', section: 'Kit de Primeiros Socorros: Como Desfazer Ações' },
        { title: 'Cenário 2: Cancelar um merge com conflito', command: 'git merge --abort', explanation: 'Se você tentar um "git merge" e se deparar com conflitos que não sabe resolver, este comando é seu botão de pânico. Ele cancela o merge e retorna seu projeto ao estado anterior, como se nada tivesse acontecido.', section: 'Kit de Primeiros Socorros: Como Desfazer Ações' },
        { title: 'Cenário 3: Reverter um merge já concluído na "main"', command: '# 1. Encontre o HASH do commit de merge que você quer reverter\ngit log --oneline\n\n# 2. Crie um novo commit que desfaz o merge anterior\ngit revert -m 1 <hash_do_commit_de_merge>', explanation: 'Este é o comando para o arrependimento final. Se você já integrou uma funcionalidade na main e a enviou para o GitHub, mas descobriu um grande problema, use este comando. O "git revert" cria um NOVO commit que desfaz todas as alterações do merge, mantendo o histórico intacto e seguro.', section: 'Kit de Primeiros Socorros: Como Desfazer Ações' },
        { title: 'Cenário 4: Resetar o Repositório Remoto (Avançado)', command: '# 1. Cria uma nova branch "órfã", sem nenhum histόrico\ngit checkout --orphan nova_main\n\n# 2. Cria um commit inicial vazio para estabelecer a nova história\ngit commit --allow-empty -m "Initial commit"\n\n# 3. Renomeia a branch local para "main" para consistência\ngit branch -M main\n\n# 4. Força o envio da sua nova "main" local para a remota, apagando tudo que estava lá\ngit push origin main --force', explanation: 'CUIDADO: Este é um processo destrutivo que apaga todo o histórico do seu repositório no GitHub. É útil para começar um projeto do zero em um repositório existente ou para corrigir erros graves no histórico.', section: 'Kit de Primeiros Socorros: Como Desfazer Ações' }
    ];
